...
type
  ...
  public
    constructor Create(aCodigo, aCodigoProfessor, aCodigoDisciplina: Integer); // Construtor para inicializar uma nova instância de TTurma com código, código do professor e código da disciplina.
    property Codigo: Integer read FCodigo write FCodigo; // Propriedade de acesso para o código da turma (leitura e escrita).
    property CodigoProfessor: Integer read FCodigoProfessor write FCodigoProfessor; // Propriedade de acesso para o código do professor (leitura e escrita).
    property CodigoDisciplina: Integer read FCodigoDisciplina write FCodigoDisciplina; // Propriedade de acesso para o código da disciplina (leitura e escrita).
    function ToString: string; override; // Sobrescreve o método padrão para retornar uma representação em string do objeto.
  end;

  // Controlador para gerenciar as operações de CRUD da entidade Turma.
  TTurmaControlador = class
  private
    FLista: TObjectList<TTurma>; // Gerencia uma lista de objetos TTurma, liberando a memória automaticamente.
    FArquivo: string; // Armazena o nome do arquivo onde os dados serão persistidos.
    function BuscarIndice(aCodigo: Integer): Integer; // Função auxiliar para encontrar o índice de uma turma na lista através do código.
    function ObterProximoCodigo: Integer; // Função auxiliar que encontra e retorna o próximo código sequencial disponível.
    function ExisteCombinacao(aCodigoProfessor, aCodigoDisciplina: Integer): Boolean; // Verifica se já existe uma turma com a combinação de professor e disciplina fornecida.
  public
    ...
    function Incluir(aCodigo: Integer; aCodigoProfessor, aCodigoDisciplina: Integer): Boolean; // Adiciona uma nova turma à lista e salva os dados no arquivo.
